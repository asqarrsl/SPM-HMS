...
const { isLoggedIn, isReceptionist,isIdValid, validateBooking, validatePayment } = require("../../middleware");

router
  .route("/")
  .get(isLoggedIn, isReceptionist, bookingController.index)
  .post(isLoggedIn, isReceptionist, validateBooking, catchAsync(bookingController.store));
...
router.route("/reciept/:id").get(isLoggedIn, isReceptionist,isIdValid, catchAsync(bookingController.reciept));

router.route("/payment")
.get(isLoggedIn, isReceptionist, bookingController.payment)
.post(isLoggedIn, isReceptionist, validatePayment, catchAsync(bookingController.pay));
...
router
  ...
  .put(isLoggedIn, isReceptionist,isIdValid, catchAsync(bookingController.update))
