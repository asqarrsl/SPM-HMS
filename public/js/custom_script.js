$(function () {
  ...
  function total(total_amount_amm,total_amount_pak,check_in,check_out) {
    ...
    total = (parseFloat(total_amount_amm) + parseFloat(total_amount_pak))*days;
    return total;
  }
  ...
  if(edit_booking.length){
    ...
    var init_pak_val =  parseFloat($('input[name="booking[package]"]:checked').attr("data-prices"));
    ...
    $('.customer-amminities').on('change',function (e) {
      ...
      $("#booking-total").html("Total : " + total(total_amount_amm,total_amount_pak,check_in,check_out))
      $("#total_val").val(total(total_amount_amm,total_amount_pak,check_in,check_out))
    });

    $('.customer-package').on('change',function (e) {
      total_amount_pak = (parseFloat($(this).attr("data-price"))==0)?parseFloat($(this).attr("data-price")):parseFloat($(this).attr("data-price")) - init_pak_val ;
      $("#booking-total").html("Total : " + total(total_amount_amm,total_amount_pak,check_in,check_out))  
      $("#total_val").val(total(total_amount_amm,total_amount_pak,check_in,check_out))  
    });
  }
